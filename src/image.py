...
class RasterImage():
    def __init__ (self, imageinfo=[], map_transforms=None,
                  dataset=None, region=None, resolution=None, folder=None, 
                  n_neighbors=None, n_rows=None, n_cols = None):
        # imageinfo = object with dataname, regionname, layername, n_neighbors, resolution
        # optional: n_rows, n_cols, folder
        # transforms = MapTransforms
        
        # Names
        self.folder = folder if folder else (imageinfo['folder'] if 'folder' in imageinfo else 'Figs/')
        self.dataset = dataset if dataset else (imageinfo['dataset'] if 'dataset' in imageinfo else '')
        self.resolution = resolution if resolution else (imageinfo['resolution'] if 'resolution' in imageinfo else '')
        self.region = region if region else (imageinfo['region'] if 'region' in imageinfo else '')
        
        # Dimension
        self.n_rows = n_rows if n_rows else (imageinfo['n_rows'] if 'n_rows' in imageinfo else 0)
        self.n_cols = n_cols if n_cols else (imageinfo['n_cols'] if 'n_cols' in imageinfo else 0)
        self.n_nodes = self.n_rows * self.n_cols
        self.n_neighbors = n_neighbors if n_neighbors else (imageinfo['n_neighbors'] if 'n_neighbors' in imageinfo else 0)
