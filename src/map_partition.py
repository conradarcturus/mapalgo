...
def getImageBase(maps, nodes_bg_value=None, nodes_bg_colormap='hashed', nodes_border=None):
    image = map_image.RasterImage(maps['elevation']) \
        .addLayer('base', 1)
...
def generateBaseMaps(params, display_and_save_image=True):
    ...
    nodes_neighbors = map_transforms.getNodesNeighbors(
...
def drawGlobalPathParentGradient(params, maps, merges):
...
def getLocaleAdjacencyList(map_params, merges, verbose=False):
    ...
    if verbose:
        ...
        print((
              ...
              .format(
                  ...
                  locale=map_params['locale_label'],
              ))
...
def drawDivisionsBreakingUpBridgesByInterface(
    ...
    locales_adjacency_list = getLocaleAdjacencyList(params, merges) # start from scratch because we will actively edit it
...
def drawPartitionsForRegion(map_params, print_stats=False, draw_all_images=False):

    # Compute locales
    [maps, nodes_neighbors] = generateBaseMaps(map_params, display_and_save_image=draw_all_images)
    
    # Compute merge points between locales
    merges = getDivisionMergePoints(map_params, maps, nodes_neighbors, draw_and_save_image=draw_all_images)
    if print_stats:
        printEarlyDivisionStats(map_params, merges, maps['locale'].getDataFlat())

    # Compute paths between locales
    nodes_path = getPaths(map_params, maps, merges, draw=draw_all_images)
    if draw_all_images:
        drawGlobalPathParentGradient(map_params, maps, merges)
    nodes_path_interface = getPathInterfaceType(map_params, maps, merges, display_image=draw_all_images)
    locales_adjacency_list = getLocaleAdjacencyList(map_params, merges, verbose=print_stats)

    # Compute divisions divided between sealevel
    interfaces_to_split = ['LSL'] if map_params['flow_direction'] == 'up' else ['LSS', 'SSL']
    compare_across_full_path = map_params['flow_direction'] == 'up' 
    _ = drawDivisionsBreakingUpBridgesByInterface(
        map_params, maps, merges, nodes_path_interface, 
        compare_across_full_path = compare_across_full_path, interfaces_to_split=interfaces_to_split, 
        print_filenames=True, display_images=draw_all_images, final_analysis_filename=True)
    
    return [map_params, maps, nodes_neighbors, merges]
